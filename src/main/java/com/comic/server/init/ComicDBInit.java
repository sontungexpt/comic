...
// comicCategoryRepository.findByName(categoryName).orElse(null);
...
//           Comic.builder()
//               .name(name)
//               .alternativeNames(originalNames)
//               .status(comicStatus)
//               .categoryIds(categoryIds)
//               .thumbnailUrl(
//                   "https://otruyenapi.com/uploads/comics/" + (String) jsonMap.get("thumb_url"))
//
// .originalSource(Source.builder().id(id).slug(slug).name(SourceName.OTRUYEN).build())
//               .build();
